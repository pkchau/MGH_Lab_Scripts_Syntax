...
stimulus_properties = subjectID,string, num_stim,string, pic_stim,string, interference,string, emotion,string, targ_buttons,string, stim_dur,string;
...
trial {
	...
	stimulus_event {
		...
		duration = 9999; #Will replace with random durations
	} msit_iaps_event;
} msit_iaps_trial;
...
#Set array of varying Stim durations (900,1000,1100,1200,1300,1400) NOTE: request duration - 8 to adjust for refresh rate
array<int> stimdur_set[3] = {592, 792, 992, 1092, 1292, 1592};

array<int> stimdur_array[0];
loop int i = 1 until i > num_trials/stimdur_set.count()
begin;
	loop int x = 1 until x > stimdur_set.count()
	begin
		if (stimdur_set.count() == num_trials) then
			break;
		end;
		stimdur_array.add(stimdur_set[x]);
		x = x + 1;
	end;
	i = i + 1;
end;
...
begin
	...
	msit_iaps_event.set_event_code(logfile.subject() + "," + num.caption() + "," + pics_array[randomizer_array[x]].filename().substring(71,8) + "," + num.description() + "," + pics_array[randomizer_array[x]].description()+ "," + string(targ_buttons[1]) + ";" + string(targ_buttons[2]) + "," + string(stimdur_array[randomizer_array[x]]));
