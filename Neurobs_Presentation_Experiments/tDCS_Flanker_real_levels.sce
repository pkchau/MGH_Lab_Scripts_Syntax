...
#NOTE: we have a stimuli ratio of 2 Congruent: 1 Incongruent to "build a tendency towards congruent responses" (based on Dr. Avram Holmes' programmed E-prime Flanker)
int num_inc = num_trials/3;
int num_con = num_trials/3*2 + 1;
int num_all_inc = num_inc*num_levels; 
int num_all_con = num_con*num_levels;

array<text> flankers_only[0];
loop int i = 1 until i > num_all_con/flankers_set.count() + 1 #Note: may need to add/subtract 1 if #s don't divide out evenly
begin
	loop int x = 1 until x > flankers_set.count()
	begin
		if (flankers_only.count() == num_all_con) then
			break;
		end;
...
term.print_line(flankers_only.count());

loop int i = 1 until i > num_all_inc/flankers_set.count() + 1 #Note: may need to add/subtract 1 if #s don't divide out evenly
begin
	loop int x = 1 until x > flankers_set.count()
	begin
		if (flankers_only.count() == num_all_inc) then
			break;
		end;
		flankers_only.add(flankers_set[x]);
		x = x + 1;
...
#Create array w/ flankers + target stim and add con stim first  
array<text> flankers_target[0];
loop int i = 1 until i > num_all_con/con_target_set.count() + 1 #Note: may need to add/subtract 1 if #s don't divide out evenly
begin;
...
#Add inc stimuli
loop int i = 1 until i > num_all_inc/inc_target_set.count() +1 #Note: may need to add/subtract 1 if #s don't divide out evenly
begin;
...
array<int> ITIs[0];
loop int i = 1 until i > num_all_trials/ITI_set.count()	+ 1 #Add 1 b/c 32/3 doesn't divide evenly and will round down
begin;
...
loop int b = 1 until b > num_blocks
	...
	begin
		...
		flankersonly.set_duration(flankerdur_set[level]);
